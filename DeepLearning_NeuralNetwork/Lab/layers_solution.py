...
def svm_loss(x, y):
    ...
    N = x.shape[0]
    C = x.shape[1]
    score_at_label = x[np.arange(N), y]
    margin = x - (score_at_label * np.ones((C, N))).T + np.ones((N, C))
    label_matrix = np.zeros((N, C))
    label_matrix[np.arange(N), y] = 1
    dx = (margin > 0).astype(float)
    dx -= (label_matrix.T * np.sum((margin > 0), axis=1)).T
    loss = np.sum((margin > 0) * margin) - N
    loss /= N 
    dx /= N
    # pass
